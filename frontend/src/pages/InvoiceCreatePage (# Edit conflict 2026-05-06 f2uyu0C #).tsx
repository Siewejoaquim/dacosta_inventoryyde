...
export const InvoiceCreatePage: React.FC = () => {
  ...
  useEffect(() => {
    api.get('/products').then((res) => {
      ...
      setItems([{ productId: res.data[0]?.id ?? '', quantity: 1, unitPrice: res.data[0]?.sellingPrice ?? 0 }]);
    });
  }, []);
  ...
  const addRow = () => {
    setItems((prev) => [...prev, { productId: products[0]?.id ?? '', quantity: 1, unitPrice: products[0]?.sellingPrice ?? 0 }]);
  };
